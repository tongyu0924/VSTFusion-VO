...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc1 = nn.Linear(3072, 512)  # 減少到中間維度
        self.gelu1 = nn.GELU()
        self.fc2 = nn.Linear(512, 256)
        self.gelu2 = nn.GELU()
        self.fc3 = nn.Linear(256, 6)    # 最終輸出層
    ...
    def forward(self, x, depth, flow):
        """Forward function."""
        x = self.patch_embed(x)
        depth = self.depth_embed(depth)
        x = x + depth
        ...
        x = torch.flatten(x, start_dim=1)  # Flatten to shape [batch_size, embed_dim]
        x = self.fc1(x)  # Linear layer for 6DoF output
        x = self.gelu1(x)
        x = self.fc2(x)
        x = self.gelu2(x)
        x = self.fc3(x)
